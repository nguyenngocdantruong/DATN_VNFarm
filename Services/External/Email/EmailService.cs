...
namespace VNFarm.ExternalServices.Email
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<bool> SendOrderPaymentSuccessEmailAsync(string to, string customerName, OrderResponseDTO order, PaymentResult paymentResult)
        {
            try
            {
                string subject = $"Thanh toán đơn hàng thành công #{paymentResult.PaymentId}";
                string body = _emailTemplates.GetPaymentSuccessTemplate(customerName, order.OrderCode, order.TotalAmount,
                    paymentResult.PaymentMethod, paymentResult.PaymentId.ToString(), paymentResult.Timestamp.ToString());
                return await SendEmailAsync(to, subject, body);
            }
            catch (Exception ex)
            {
                _logger.LogError($"Failed to send order payment success email to {to}. Error: {ex.Message}");
                return false;
            }
        }

        public async Task<bool> SendUserActiveEmailAsync(string to, string customerName, bool isActive)
        {
            try
            {
                string subject = isActive ? "[VNFarm] Thông báo: Tài khoản của bạn đã được kích hoạt" : "[VNFarm] Thông báo: Tài khoản của bạn đã bị vô hiệu hóa";
                string body = _emailTemplates.GetUserActiveTemplate(customerName, isActive);
                return await SendEmailAsync(to, subject, body);
            }
            catch (Exception ex)
            {
                _logger.LogError($"Failed to send user active email to {to}. Error: {ex.Message}");
                return false;
            }
        }

        public async Task<bool> SendDiscountCreatedEmailAsync(string to, string customerName, string discountCode, string discountDescription, DateTime startDate, DateTime endDate, string discountAmount, int remainingQuantity)
        {
            try
            {
                string subject = $"Mã giảm giá mới đã được tạo: {discountCode}";
                string body = _emailTemplates.GetDiscountCreatedTemplate(customerName, discountCode, discountDescription, startDate, endDate, discountAmount, remainingQuantity);
                return await SendEmailAsync(to, subject, body);
            }
            catch (Exception ex)
            {
                _logger.LogError($"Failed to send discount created email to {to}. Error: {ex.Message}");
                return false;
            }
        }
    }
